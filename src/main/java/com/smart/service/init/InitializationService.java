...
public class InitializationService {
    ...
    public void createClient() {
        restTemplate.exchange("http://auth-service/auth/api/v1/client/" + applicationProperties.getName(),
                HttpMethod.PUT, new HttpEntity<>(null), ResponseEntity.class);
    }

    public void createClientRoles() {
        restTemplate.exchange("http://auth-service/auth/api/v1/role/batch/" + applicationProperties.getName(),
                HttpMethod.POST, new HttpEntity<>(beanService.getAllControllers()), ResponseEntity.class);
    }

    public void createRealmRole() {
        restTemplate.exchange("http://auth-service/auth/api/v1/role/", HttpMethod.POST,
    ...
    public void assignClientRolesToRealmRole() {
        restTemplate.exchange(
                "http://auth-service/auth/api/v1/role/composite/" + applicationProperties.getAdminRole() + "/"
